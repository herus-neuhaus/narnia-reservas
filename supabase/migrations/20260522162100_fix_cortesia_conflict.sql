-- Upsert in approve_complimentary_ticket
...
BEGIN
  ...
  IF p_status = 'approved' THEN
    ...
    -- Criar reserva (Entrada liberada, mas não entrou fisicamente) ou Atualizar
    ...
    ON CONFLICT (reservation_date, cpf) WHERE cpf IS NOT NULL DO UPDATE
    SET type = 'cortesia',
        status = 'confirmed',
        payment_status = 'not_required',
        payment_amount = 0,
        check_in_status = CASE WHEN public.reservations.check_in_status = 'entered' THEN 'entered' ELSE 'pending' END;
  END IF;
