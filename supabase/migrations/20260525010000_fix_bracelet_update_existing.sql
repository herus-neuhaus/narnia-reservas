...
BEGIN
  ...
  IF v_age < 18 THEN
    RETURN jsonb_build_object('success', false, 'error', 'UNDERAGE', 'message', 'O cliente e menor de idade (18-). Acesso negado.');
  END IF;
  ...
  IF FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'DUPLICATE', 'message', 'O cliente ja comprou pulseira ou ja tem entrada hoje.');
  END IF;
