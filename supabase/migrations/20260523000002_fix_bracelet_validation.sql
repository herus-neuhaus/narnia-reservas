...
BEGIN
  ...
  -- 5. Validação de Duplicidade / Conflito de Reservas (NOVA REGRA)
  ...
  IF FOUND THEN
    -- Se tem uma lista pendente (expirada ou não), permite a venda e ignora o bloqueio
    IF v_existing_res.type = 'lista' AND v_existing_res.entered_at IS NULL AND v_existing_res.check_in_status = 'pending' THEN
      NULL; -- Passagem silenciosa (será atualizada no passo 7)
    ELSE
      -- Bloqueia caso o cliente já esteja na casa ou possua Cortesia/Camarote/Mesa
      RETURN jsonb_build_object(
        'success', false, 
        'error', 'DUPLICATE', 
        'message', 'O cliente já possui uma entrada realizada ou uma reserva ativa (' || v_existing_res.type || ') para hoje.'
      );
    END IF;
  END IF;
  ...
  -- 7. Criar ou Atualizar a Reservation / Entrada
  IF EXISTS (SELECT 1 FROM public.reservations WHERE customer_id = v_customer_id AND reservation_date = p_event_date AND check_in_status = 'pending') THEN
    UPDATE public.reservations
    SET type = 'pulseira',
        check_in_status = 'entered',
        entered_at = NOW() AT TIME ZONE 'America/Porto_Velho',
        payment_status = 'paid',
        payment_amount = v_payment_amount,
        ticket_batch_id = v_batch_id,
        photo = COALESCE(p_photo, photo)
    WHERE customer_id = v_customer_id AND reservation_date = p_event_date AND check_in_status = 'pending';
  ELSE
